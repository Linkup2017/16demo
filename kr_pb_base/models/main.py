...
class AccountMove(models.Model):
    ...
    def action_post(self):
        """ hometax.move.bill id 생성을 위한 account.move의 action_post inherit """
        ...
        # items = self.env["account.move.line"].sudo().search([("move_id", "in", self.id)])

        for item in self.invoice_line_ids:
            if item:
                # bill_line = self.env['hometax.move.bill.line'].create({
                #     'name': item.name,
                ...
                # })
                _logger.info('----------------- items 가격 %s ----------------', item.price_unit)
                _logger.info('----------------- items 수량 %s ----------------', item.quantity)
                _logger.info('----------------- items 계정 %s ----------------', item.account_id)
                _logger.info('----------------- items 라벨 %s ----------------', item.name)
                _logger.info('----------------- items 품목 %s ----------------', item.product_id)
                _logger.info('----------------- items 품목 %s ----------------', item.move_id)



                _logger.info('----------------- items 호출됨 %s ----------------', self.name)
                _logger.info('----------------- items 호출됨 %s ----------------', self.id)
                _logger.info('----------------- items 호출됨 %s ----------------', self.invoice_date)
                _logger.info('----------------- items 공급가액 %s ----------------', self.amount_untaxed)
            search_line = self.env['hometax.move.bill.line'].search([])
            # bill_id = self.env['hometax.move.bill'].create({
            #     'move_id': self.id,
            #     'name': self.name,
