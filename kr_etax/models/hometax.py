...
class ResPartner(models.Model):
    ...
    #     if company_id:
    #         self.ceo_name = company_id.ceo_name
...
# LJY ,ondelete={'etax': 'set default'
class AccountTax(models.Model):
    _inherit = "account.tax"
    # type_tax_use = fields.Selection(selection_add=[("etax", "e-Tax")], default="none")
    e_tax_is = fields.Boolean(string="Is e-Tax", default=False)
    e_tax_code = fields.Selection(
        [("과세", "과세10%"), ("면세", "면세")], string="e-Tax Type", default="과세"
    )
...
class ResCompanyInherit(models.Model):
    ...
    tax_code = fields.Selection(
        [("과세", "과세10%"), ("면세", "면세")], string="Tax Type", default="과세"
    )
...
class HometaxMove(models.Model):
    ...
    def _get_tax_id(self):
        etax = self.env["res.company"].search([("etax_on", "=", True)], limit=1)
        if etax:
           return etax.id
    ...
    taxType_value = fields.Many2one("account.tax", string="과세 값", default=_get_tax_id)
    # supplier_id = fields.Many2one("res.partner", string="공급자", default=_get_supplier, required=True)
    supplier_id = fields.Many2one("res.partner", string="공급자")
    # check_id = fields.Many2one("res.partner", string="공급자 확인용", default=_get_supplier, required=True)
    check_id = fields.Many2one("res.partner", string="공급자 확인용")

    MgtKeyType = fields.Selection(
        selection=[
            ("sell", "매출"),
            ("buy", "매입"),
            ("trustee", "위수탁")
    ...
    supplier_vat_no = fields.Char(string="등록번호")  # 사업자번호
    ...
    date_finish = date_start + timedelta(days=-30)
    ...
    def _onchange_tax_type(self):
        ...
        if self.taxType1 == 'invoiceeType1':
...
class HometaxMoveLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for item in self:
            ...
            if item.move_id.taxType_value.amount > 0:
                taxrate = 1 / item.move_id.taxType_value.amount
...
class HometaxMoveBill(models.Model):
    _name = "hometax.move.bill"

    name = fields.Char(string='Name')
    move_id = fields.Many2one('account.move', string='Account Move ID')
    amount_untaxed = fields.Float(string='Account Move Amount')
    hometax_id = fields.Many2one('hometax.move', string='HomeTax Move ID')
    invoice_date = fields.Date(string='Invoice/Bill Date')
    line_ids = fields.One2many('hometax.move.bill.line', 'line_id', string='Bill Line')


class HometaxMoveBillLine(models.Model):
    _name = "hometax.move.bill.line"

    line_id = fields.Many2one('hometax.move.bill', string='Bill ID')
